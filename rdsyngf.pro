pro rdsyngf,name,lamb,flux,bb,flux2
;---------------------------
...
if (strpos(name,'gz') ge 0) then begin
 spawn,string("gunzip -c "+name+' | egrep -v -i nan > ~/tmp/rdsyn_7.tmp'),ierr
 openr, 1,"~/tmp/rdsyn_7.tmp"
endif

if (strpos(name,'bz2') ge 0) then begin
 spawn,string("bunzip2 -c "+name+' | egrep -v -i nan > ~/tmp/rdsyn_7.tmp'),ierr
 openr, 1,"~/tmp/rdsyn_7.tmp"
endif
...
flux2=flux2(il)
bb=bb(il)
spawn,string("rm ~/tmp/rdsyn_7.tmp"),ierr
end
