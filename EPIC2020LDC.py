...
uncert = np.max(np.abs(logg - np.log10(g)))
print logg - np.max(np.log10(g))
print logg - np.min(np.log10(g))
...
sc = LDPSetCreator(teff=[3785,185],logg=[4.75,0.22],z=[-0.13,0.15],filters=filters)
